...
NETWORK_CONNECTED="#a4cc91"
NETWORK_DISCONNECTED="#ff0938"
...
do
	...
	CONNECTION=$(echo $line | sed -E 's/.+connection\":(true|false),?\"?.+?\}/\1/')
	if [ "$CONNECTION" == "true" ] 
		then COLOR=$NETWORK_CONNECTED 
		else COLOR=$NETWORK_DISCONNECTED 
	fi
	echo "<wpt lat=\"$LAT\" lon=\"$LON\"><time>$DATETIME</time><name><![CDATA[$SPEED]]></name><extensions><color>$COLOR</color></extensions></wpt>" >> /tmp/wpt-$2
